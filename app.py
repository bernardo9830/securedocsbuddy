...
<style>
  * { box-sizing: border-box; }
  body {
    ...
    animation: sfondo 16s ease infinite;
  ...
  .hbtn { background: #ffffff22; color: #fff; border: none;
  ...
  #domanda { flex: 1; padding: 12px 16px; border: 1px solid #d1d5db; border-radius: 24px;
  ...
  #invia:hover { transform: scale(1.05); }
  #invia:disabled { opacity: .5; cursor: default; transform: none; }
...
<body>
  <div class="card">
    ...
    <div id="pannello" class="pannello">
      <div class="pannello-head">
        <strong>Risposte precedenti</strong>
        <button onclick="toggleMemoria()">X</button>
      </div>
...
<script>
  ...
  let storico = JSON.parse(localStorage.getItem('docbuddy_storico') || '[]');

  function rigaUtente(testo) {
    const riga = document.createElement('div');
    riga.className = 'riga riga-utente';
    const b = document.createElement('div');
    b.className = 'msg utente';
    b.textContent = testo;
    riga.appendChild(b);
    chat.appendChild(riga);
    chat.scrollTop = chat.scrollHeight;
  }

  function rigaBot() {
    const riga = document.createElement('div');
    riga.className = 'riga riga-bot';
  ...
  function benvenuto() {
    const b = rigaBot();
    b.textContent = 'Ciao! Sono DocBuddy. Posso rispondere, fare un riassunto Word, creare 2 slide o citarti le fonti.';
  }
  benvenuto();
  ...
  async function invia() {
    ...
    const bolla = rigaBot();
    bolla.className = 'msg bot typing';
    ...
    try {
      ...
      bolla.className = 'msg bot';
      bolla.textContent = dati.risposta;

      if (dati.download) {
        const a = document.createElement('a');
        a.href = dati.download;
      ...
      localStorage.setItem('docbuddy_storico', JSON.stringify(storico));
    } catch (e) {
      bolla.className = 'msg bot';
      bolla.textContent = 'Errore: ' + e;
    } finally {
  ...
  async function svuotaMemoria() {
    ...
    try {
      await fetch('/svuota-memoria', { method: 'POST' });
    } catch (e) { /* ignoro errori di rete: pulisco comunque il browser */ }
    storico = [];
    localStorage.removeItem('docbuddy_storico');
    chat.innerHTML = '';
    benvenuto();
  ...
  function toggleMemoria() {
    ...
    if (storico.length === 0) {
      lista.innerHTML = '<div class="vuoto">Nessuna risposta ancora.</div>';
    } else {
      lista.innerHTML = '';
